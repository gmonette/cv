...
\end{align*}
where the subscript $\mathbf{j} = (i_{j1}, \ldots, i_{jn_j})^\top$ represents the vector of indices for the $n_j \approx n/k$ cases in the $j$th fold, $j = 1, \ldots, k$. The negative sign in $\mathbf{V} = - \mathbf{X}_\mathbf{j}$ reflects the *removal*, rather than addition, of the cases in $\mathbf{j}$. 

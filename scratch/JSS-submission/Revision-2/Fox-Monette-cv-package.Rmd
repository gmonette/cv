...
The approach taken by \pkg{scikit-learn} for independent observations is similar to that in the default \code{cv()} function in the \pkg{cv} package. The approach for hierarchical data is similar, but not identical, to that in \pkg{cv} (as described in Section \ref{cross-validating-mixed-effects-models}). Our \code{cv()} package doesn't currently provide for cross-validating time-series models.

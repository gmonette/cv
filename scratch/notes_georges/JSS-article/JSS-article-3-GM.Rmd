...
```
Figure \ref{fig:plot1} display scatterplots of each data set showing estimated 50% concentration ellipses for each cluster.[^lattice]
...
```
The fixed-effects predictions from these models are displayed in Figure \ref{fig:plot-fits-fixed} along with a summary of the data using 50% concentration ellipses.  Figure \ref{fig:plot-fits-blups} similarly displays predicted BLUPs. 
```{r plot-fits-fixed,echo=FALSE}
#| out.width = "100%",
#| fig.height = 6,
#| fig.cap = "Fixed-effect predictions using each model applied to data sets with varying between- and within-patient variance ratios.  The top row shows summaries of the within-patient data using estimated 50% concentration ellipses."
{
  xyplot(y ~ x |SD_ratio * panel, Data,
         ...
         ylab = list('fixed-effect predictions',cex=.7)) + 
...
```
<!-- The BLUPs from these models, shown in Figure \ref{fig:plot-fits-blups}, are produced as follows: -->
```{r plot-fits-blups,echo=FALSE}
#| out.width = "100%",
#| fig.height = 6,
...
{   xyplot(y ~ x | SD_ratio * panel, Data,
         ...
         par.strip.text = list(cex = 0.6),
         scales = list(x = list(alternating = FALSE)),
         ylab = list('fixed- and random-effect predictions (BLUPS)', cex = 0.7)) +
    glayer(panel.ellipse(..., center.pch = 16, center.cex = 0.5,  
                         level = 0.5),
           panel.abline(a = 10, b = 1)) +
...
.opts <- options(warn = -2) 
```
```{r cross-validation,cache=TRUE,message=FALSE}
...
```
For a given dataset, we can plot the results for a list of models using the plot method for `"cvList"` objects. For example, this code produces Figure \ref{fig:plot-cv-example}.
```{r plot-cv-example}
#| out.width = "60%",
#| fig.height = 3.5,
...
xyplot(cases + clusters ~ model|SD_ratio_i, dsummary,
       auto.key = list(space = 'top', reverse.rows = T, columns = 2), 
       type = 'b', ylab = 'CV criterion (MSE)',
       par.strip.text = list(cex = 0.7),
       layout= c(Nv, 1),
       par.settings =
         list(superpose.line=list(lty = c(1, 2), lwd = 2),
              superpose.symbol=list(pch = 15:16, cex = 1)),
       scales = list(y = list(log = TRUE), 
                     x = list(alternating = F, rot = 45))) 
```

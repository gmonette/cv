...
```
Figure \ref{fig:plot1} displays scatterplots of each data set showing estimated 50% concentration ellipses for each cluster.[^lattice]
...
```
For a given dataset, we can plot the results for a list of models using the plot method for `"cvList"` objects. 
For example, a comparison cluster-based cross-validation using fixed effects for the models fit to the third dataset (with "SD ratio = 6 / 2.5") is shown in Figure \ref{fig:plot-cv-example}.
```{r plot-cv-example,echo=FALSE}
#| out.width = "60%",
#| fig.height = 3.8,
#| fig.cap = "10-fold cluster-based cross-validation comparing random intercept mixed models with varying fixed effects."
plot(cvs_clusters[[3]], main="Comparison of Fixed Effects",)
```
The results for all datasets, using both cluster-based and case-based cross-validation, are assembled and shown in Figure \ref{fig:cross-validation-data-plot}.
...
xyplot(clusters + cases ~ model|SD_ratio_i, dsummary,
       auto.key = list(space = 'top', reverse.rows = T, columns = 2, cex = 0.7), 

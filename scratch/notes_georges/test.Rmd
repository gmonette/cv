---
title: "Cross-validation of regression models: mixed model examples (edited)"
...
# Parameters:
set.seed(9665) # set.seed(123) # set.seed(9693)
...
plot <- xyplot(y ~ x, data=Data, group=group,
               ylim=c(0, 20),
...
Data_long <- reshape(Data, direction = "long", sep = ".", 
              timevar = "effect", varying = grep("\\.", names(Data)))
...
cvs_clusters <- cv(modlist, data=Data, cluster="group", seed=4576)


{
  
  # play
  cv0 <- replicate(10,cv(fit0))
  cv1 <- replicate(10,cv(fit1))
  cv2 <- replicate(10,cv(fit2))
  ...
  cvs_cases <- cv(modlist, data=Data)
  cvs_clusters <- cv(modlist, data=Data, cluster="group")

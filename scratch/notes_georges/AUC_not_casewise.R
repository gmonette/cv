...
#' the vector of prediction
#' criteria
#' $\mathbf{\hat{y}} = (\hat{y}_1, ..., \hat{y}_1)'$
#' and the observed responses in validation set,
#' $\mathbf{y} = (y_1, ..., y_n)'$
#' with $y_i \in \{0,1\}, i = 1,...,n$.
#' To construct the ROC curve, only the ordering of
#' the values in $\mathbf{\hat{y}}$ is needed.
#' Thus, assuming that there are no ties and reordering
#' observations if necessary, we can set $\mathbf{y} = (1,2,...,n)'$.
#'
#' If the AUC can be expressed as the case-wise mean or sum of a function $cv(\hat{y}_i,y_i)$
...
#' if could be set to 0 if all $y$s are 0, and to 1 if all $y$s are 1.
#' If AUC is considered to be undefined in these cases, we are have
#' $2^n - 2$
...
#' If $n=4$, we show below that there no consistent solutions
#' in either case.
#'
...
#'
#' We generate all possible $\mathbf{y}$ of length $n=3$ as rows of the matrix
#' $\boldbf{Y}$ with the following function:
#'
Ymat <- function(n, exclude_identical = FALSE) {
    stopifnot(n > 0 && round(n) == n)    # n must be a positive integer
    if(n == 1) return(cbind(c(0, 1)))
    prev <- Ymat(n - 1)
    ret <- rbind(cbind(0, prev), cbind(1, prev))
...
Ymat(3)
#'
#' If we exclude $\mathbf{y}$ with identical values of $y$s:
Ymat(3, TRUE)
#'
#' The following shows $\mathbf{Y}$ with corresponding values of AUC:
cbind(Ymat(3), AUC = apply(Ymat(3), 1, AUC))
#'
#' Observe that the values of $cv(\hat{y}_i, y_i)$ to express
#' AUC as a sum of case-wise values are the solutions of the linear
#' equation (\ref{eq:cw}):
#' $$
...
#' \end{aligned}
#' \end{bmatrix}= AUC(\mathbf{\hat{Y}},\mathbf{Y})
#' $$
...
#' 1 to $n$.
#'
#' We can test whether these equations have a solution for any $n$
...
#' But, if identical $y$s are included, the equations are not consistent:
...
#'
#' We have demonstated that a widely used criterion, AUC, cannot be cross-validated

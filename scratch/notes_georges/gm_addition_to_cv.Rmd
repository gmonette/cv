...
names(modlist) <- c('lmer:~1','lme:~1','lmer:~x','lme:~x','lmer:~x+xm','lme:~x+xm')
modlist <- do.call(models, modlist)
```
We can compare estimated fixed-effect coefficients.

```{r, message=FALSE,warning=FALSE}
tbl <- do.call(compareCoefs, modlist)
colnames(tbl) <- names(modlist)
tbl[] <- round(tbl,4)
tbl[is.na(tbl)] <- ''
...
#| out.width = "100%",
#| fig.height = 6,
#| fig.cap = "Cross-validation using lmer and lme comparing random intercept models with varying fixed effects and with case- and cluster-based resampling."

cvs_cases <- cv(modlist, data = dd)
cvs_clusters <- cv(modlist, data = dd, cluster = 'group')

cvs_cases <- cvs_cases %>% lapply(rbind) %>% do.call(rbind,.) %>% as.data.frame
cvs_clusters <- cvs_clusters %>% lapply(rbind) %>% do.call(rbind,.) %>% as.data.frame
...
dout <-
  within(
    ...
    {
      model <- factor(sub('lme.*:','',modlist))
      method <- factor(sub(':.*$','',modlist))
      CV <- as.numeric(as.character(`CV crit`))
      cv_method <- factor(cv_method)
    }
)

dout <- dout[order(dout$model),]

xyplot(CV ~ model , dout, groups = cv_method:method,
       par.settings =
         list(superpose.line = list(lty=c(2,3),lwd = 3),superpose.symbol=list(pch=21:24, fill='transparent', cex =1)),
       type = 'b',
       auto.key = T,

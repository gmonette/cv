...
```
The response variable in this example, \code{Attrition}, is binary. The function \code{holdout_results()} must be supplied by the user, and "accuracy" is the complement of the Bayes-rule classification error rate.
...
\end{align*}
where the subscript $\mathbf{j} = (i_{j1}, \ldots, i_{jm})^\top$ represents the vector of indices for the cases in the $j$th fold, $j = 1, \ldots, k$. The negative sign in $\mathbf{V} = - \mathbf{X}_\mathbf{j}$ reflects the *removal*, rather than addition, of the cases in $\mathbf{j}$. 

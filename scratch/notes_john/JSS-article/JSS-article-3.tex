...
\end{CodeInput}
\begin{CodeOutput}
R RNG seed set to 508110
\end{CodeOutput}
\begin{CodeOutput}
10-Fold Cross Validation
method: Woodbury
criterion: mse
cross-validation criterion = 19.23279
bias-adjusted cross-validation criterion = 19.21953
95% CI for bias-adjusted CV criterion = (15.76747, 22.67159)
...
CV for this example using \pkg{rsample}:\footnote{The example in the
  \pkg{rsample} vignette uses 10-fold rather than LOO CV, but otherwise
...
magnitude slower than \code{cv()} with \code{method="Woodbury"}, and
more than three orders of magnitude slower then \code{cv()} with

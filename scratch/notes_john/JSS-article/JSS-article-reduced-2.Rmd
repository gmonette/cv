...
## Example: CV of clusters versus individuals using contrived data
...
* Arguments required by the model-selection function: the starting \code{model} (\code{m.auto}) for predictor selection, the names of the variables---\code{predictors} and \code{response}---that are candidates for transformation, and \code{AIC=FALSE}, which specifies use of the BIC for model selection. 

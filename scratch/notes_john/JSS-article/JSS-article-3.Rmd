...
The \pkg{caret} package doesn't directly support generalized linear models fit by \code{glm()}, but it can be made to do so by defining a custom "method" for the binary logistic regression fit to the \code{attrition} data:
```{r rsample-example-using-caret, cache=TRUE}
library("caret", quietly=TRUE, warn.conflicts=FALSE)

glm_mod <- getModelInfo("glm", regex = FALSE)[[1]]
glm_mod$fit <- function(x, y, wts, param, lev, last, classProbs, ...) {
  x$Attrition <- y
  glm(Attrition ~ JobSatisfaction + Gender + MonthlyIncome,
      family = binomial, data = x)
}

mod <- train(x = attrition[, c("JobSatisfaction", "Gender",
                               "MonthlyIncome")],
             y = attrition$Attrition,
             method = glm_mod,
             trControl = trainControl(method="LOOCV"))
mod # reports complement of Bayes-rule criterion
```
In fairness, \pkg{caret} really isn't intended for this kind of simple CV computation.

...
As we explain in the vignette "Cross-validation of regression models," the `cv()` function differentiates between CV criteria that are averages of casewise components and criteria that are not. Computation of bias corrections and confidence intervals is limited to the former. We don't know whether the AUC, and hence its complement, can be expressed as the mean of casewise components, and certainly the `auc()` function in the **Metrics** package, on which our `AUCcomp()` is based, isn't written in this manner.

...
```
Notice that separate "helper" functions are defined for non-parallel and parallel computations.[^multinom-parallel] The new version of `cv.multinom()` produces the same results as the  version that calls `cv.default()`:

...
As we explain in the vignette "Cross-validation of regression models," the `cv()` function differentiates between CV criteria that are averages of casewise components and criteria that are not. Computation of bias corrections and confidence intervals is limited to the former. As far as we know, the AUC, and hence its complement, can't be expressed as the mean of casewise components, and certainly the `auc()` function in the **Metrics** package, on which our `AUCcomp()` is based, isn't written in this manner.

`cv()` looks for a `"casewise average"` attribute of the value returned by a CV criterion function. If this attribute exists and is `TRUE`, then the criterion is treated as the mean of casewise components; if the attribute is absent or `FALSE`, then it is not. If the `casewise average` attribute is `TRUE`, then `cv()` uses the unexported function `getLossFn()` to construct a function that returns the casewise components of the criterion.
...
cv:::getLossFn(mse)
```
For this scheme to work, the criterion function must assign its value to the local variable `result` (here, `result <- mean((y - yhat)^2`) and attach the `"casewise average"` attribute, set to `TRUE`, to `result`. `getLossFn()` returns a version of the criterion with the call to `mean()` and the attribute removed.
...
Applying `selectSubsets()` to the full data produces the full-data cross-validated RMSE (which we obtained previously):
...
```
Cross-validation shows that model selection exacts a penalty in RMSE. Examining the models selected for the 10 folds reveals that there is some uncertainty in identifying the predictors in the "best" model, with `Agriculture` sometimes appearing and sometimes not:

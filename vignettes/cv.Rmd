...
The following graph shows two measures of estimated error as a function of polynomial-regression degree: The root-mean-squared error ("RMSE"),[^RMSE-vs-MSE] defined as $\mathsf{RMSE} = \sqrt{\sum (y_i - \widehat{y}_i)^2/n}$, and the usual residual standard error, defined as $\widehat{\sigma} = \sqrt{\sum (y_i - \widehat{y}_i)^2/(n - p - 1)}$. The former necessarily declines with $p$ (or, more strictly, can't increase with $p$), while the latter gets slightly larger for the largest values of $p$, with the "best" value, by a small margin, for $p = 7$.

[^RMSE-vs-MSE]: Although one is just the square-root of the other, we prefer the RMSE to the MSE because the former is measured in the units of the response variable rather than in squared units.

```{r mpg-horsepower-RMSE-se}
#| out.width = "100%",
#| fig.height = 5,
...
```
The code for this graph uses the `rmse()` function from the **cv** package to compute the RMSE for each fit.
...
cv(m.auto, k="loo")
```
For LOO CV of a linear model, `cv()` by default uses the hatvalues from the model fit to the full data for the LOO updates, and reports only the CV estimate of RMSE. Alternative methods are to use the Woodbury matrix identity or the "naive" approach of literally refitting the model with each case omitted. All three methods produce exact results for a linear model (within the precision of floating-point computations):
```{r cv.lm-3}
cv(m.auto, k="loo", method="naive")

cv(m.auto, k="loo", method="Woodbury")
```
The `"naive"` and `"Woodbury"` methods also return the bias-adjusted estimate of RMSE and the full-sample RMSE, but bias isn't an issue for LOO CV.
...
Finally, we extract and graph the adjusted RMSEs for $10$-fold CV and the RMSEs for LOO CV:
```{r polynomial-regression-CV-graph}
#| out.width = "100%",
#| fig.height = 5,
#| fig.cap = "Cross-validated 10-fold and LOO RMSE as a function of polynomial degree, $p$"
...
```
Because these predictions are based on BLUPs, they are more accurate than the predictions based only on fixed effects.[^crossed-vs-nested] As well, the difference between the RMSE computed for the model fit to the full data and the CV estimates of the RMSE is greater here than for cluster-based predictions.
...
By default, `cvSelect()` performs 10-fold CV, and produces an estimate of RMSE for the model-selection procedure even *larger* than the true error variance, $\sigma^2 = 1$.
...
```
Comparing the RMSE for the regressions with the original and transformed predictors shows a advantage to the latter:
...
Anova(m.step)
```
The selected model includes three of the numeric predictors, `horsepower`, `weight`, and `acceleration`, along with the factors `year` and `origin`. We can calculate the RMSE for this model, but we expect that the result will be optimistic because we used the whole data to help specify the model
```{r RMSE-whole-selected-model}
rmse(Auto$mpg, exp(fitted(m.step)))
```
This is considerably smaller than the RMSE for the original working model:
```{r RMSE-working-model}
rmse(Auto$mpg, fitted(m.auto))
```
A perhaps subtle point is that we compute the RMSE for the selected model on the original `mpg` response scale rather than the log scale, so as to make the selected model comparable to the working model. That's slightly uncomfortable given the skewed distribution of `mpg`. An alternative is to use the median absolute error instead of the root-mean-squared error, employing the `medAbsErr()` function from the **cv** package:
